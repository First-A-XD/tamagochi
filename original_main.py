...
class FoodMenu:
    def __init__(self, game):
        ...
        self.items = [Food("Мясо", 30, "images/food/meat.png", 16),
        Food("Помои", 49, "images/food/dog food.png", 15),
        Food("VIP корм", 100, "images/food/dog food elite.png", 25, medicine_power=2),
        Food("Аспирин", 260, "images/food/medicine.png", 8, medicine_power=10)]
class MiniGame:
    def __init__(self, game):
        self.game = game
...
class Game:
    ...
    def event(self):
        for event in pg.event.get():
            ...
            # if event.key == pg.K_z:
            #     pg.time.set_timer(self.INCREASE_COINS, 0)
            # if event.key == pg.K_x:
            #     pg.time.set_timer(self.INCREASE_COINS, 200)
            if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
                self.money += self.coins_per_second
